  ...
  return lines
    ...
    .map(function (x) {
      if (x.type === md.Syntax.Html || x.type === md.Syntax.HtmlBlock) {
        var htmlNode = getHtmlHeaders({children: [x]}, start, options.heading.level.max, options.heading.level.min, false)[0];
        return htmlNode ? {
          rank: htmlNode.rank,
          href: htmlNode.href,
          name: htmlNode.name,
...
exports = module.exports = function transform(content, mode, maxHeaderLevel, minHeaderLevel, minTocItems, title, notitle, entryPrefix, processAll, updateOnly, syntax, options) {
  ...
  var headers = getMarkdownHeaders(docNodes, tocableStart, options.heading.level.max, options.heading.level.min, syntax)
    .concat(getHtmlHeaders(docNodes, tocableStart, maxHeaderLevelHtml, options.heading.level.min, true));
  ...
  if (headers.length >= options.toc.items.min && docNodes.children[docNodes.children.length - 1].loc.end.line >= minLines) {
    var tocLines = [];
    var inferredTitle = determineTitle(docNodes, tocPosition, options.toc.title.content, options.toc.title.remove, options.toc.header?.content);
    ...
    var spacer = inferredTitle ? options.toc.title.padding.before : 0
    while (spacer < options.toc.title.padding.before) {
      tocLines.push('');
    }
