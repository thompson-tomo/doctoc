...
function getMarkdownHeaders (lines, start, headingOptions, syntax) {
  ...
  return lines
    ...
    .map(function (x) {
      if (x.type === md.Syntax.Html || x.type === md.Syntax.HtmlBlock) {
        var htmlNode = getHtmlHeaders({children: [x]}, start, headingOptions.level.max, headingOptions.level.min, false)[0];
        return htmlNode ? {
          rank: htmlNode.rank,
          href: htmlNode.href,
          name: htmlNode.name,
      ...
      else if (x.type == md.Syntax.Header && x.depth >= headingOptions.level.min && (!headingOptions.level.max || x.depth <= headingOptions.level.max)) {
        return { rank :  x.depth
          , name :  extractText(x)
          , line :  x.loc.start.line
          , node :  x
...
function determineTitle(nodes, tocPosition, titleOptions, headerOptions) {
  ...
  if (title && headerOptions?.content?.includes(title)) return "";
  return title || "";
}
...
exports = module.exports = function transform(content, mode, maxHeaderLevel, minHeaderLevel, minTocItems, title, notitle, entryPrefix, processAll, updateOnly, syntax, options) {
  options = new configBuilder()
    .use(() => defaultConfig.defaults)
    .use(() => options)
    .use(() => {
      heading: {
        level: {
      ...
      toc: {
        items: {
          min: minTocItems,
          source: processAll ? 'all' : 'after',
          symbols: String(entryPrefix || '-').split(',')
        },
        title: {
          content: title,
          padding: {
            before: noTitle ? 1 : undefined
          },
          remove: noTitle
        }
      }
  ...
  var tocableStart = options.toc.items.source === 'after' ? tocPosition.end.range[1] : startPos;
  var headers = getMarkdownHeaders(docNodes, tocableStart, options.heading, syntax)
    .concat(getHtmlHeaders(docNodes, tocableStart, options.heading.level.max, options.heading.level.min, true));
  ...
  if (headers.length >= options.toc.items.min && docNodes.children[docNodes.children.length - 1].loc.end.line >= minLines) {
    var tocLines = [];
    var inferredTitle = determineTitle(docNodes, tocPosition, options.toc.title, options.toc.header);
    ...
    if (inferredTitle) {
      tocLines.push(...Array(options.toc.title.padding.before).fill(''));
      tocLines.push(inferredTitle);
    }
    tocLines.push('');
