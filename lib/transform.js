...
exports = module.exports = function transform(content, mode, maxHeaderLevel, requiredHeaders, title, notitle, entryPrefix, processAll, updateOnly) {
  if (content.indexOf(skipTag) !== -1) return { transformed: false };

  mode = mode || 'github.com';
  entryPrefix = entryPrefix || '-';

  if (isNaN(requiredHeaders)){
    requiredHeaders = 1;
  }
  ...
  if (linkedHeaders.length >= requiredHeaders) {
    toc = inferredTitle
    + titleSeparator
    + linkedHeaders
        .map(function (x) {
